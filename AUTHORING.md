...
- **`Touched` 不会因为「人站在上面」持续刷**（2026-08 实机核对）。教科书讲 debounce 都说
  「站着不动分数会疯狂往上涨」，当前 Studio 里不是这样：实测站着不动 10 秒只加 7 分、
  来回挪 13 秒 11 分、原地跳 15 秒 9 分。真实现象是**一次踩踏触发好几次**（脚 / 腿 / 身子
  每个部件各算一次，输出面板会合并成一行记 `(x6)`）。所以讲 debounce 要拿**「踩一脚」**举例
  （无 debounce 6 分 vs 有 debounce 2 分），别写「站着不动就疯狂涨」—— 单元 6 已按这个重写。
  ⚠️ 输出面板里那个重复计数是**变的**（同一段代码两次试玩拍到 x12 和 x10），课文里别写死。
